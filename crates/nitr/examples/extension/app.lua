...
-- application cannot tell `nitr.kv` (this repository's Rust code) from
...
app:get("/inventory/:item", function(req)
    return nitr.json({
        ...
        count = nitr.kv.get(req.params.item),
        requests_served = nitr.kv.get("requests"),
    })
end)
...
app:put("/inventory/:item", function(req)
    ...
    local count = nitr.kv.add(req.params.item, delta)
...
app:get("/slugify", function(req)
    ...
    return nitr.json({ title = title, slug = nitr.slug.slugify(title) })
end)

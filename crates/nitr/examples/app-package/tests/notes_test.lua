...
local resp = nitr.test.request("POST", "/api/notes", { body = '{"text":""}' })
assert(resp.status == 422, "empty note: " .. resp.status)

-- Create, then read back.
local resp = nitr.test.request("POST", "/api/notes", { body = '{"text":"from the test"}' })
assert(resp.status == 201, "create: " .. resp.status)

local resp = nitr.test.request("GET", "/api/notes")
assert(resp.status == 200, "list: " .. resp.status)
local notes = nitr.json:decode(resp.body)
